...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        {
            var process = Process.Start("CMD.exe", "/c adb install " + textBox3.Text);
            process.WaitForExit();
        ...
        {
            var process = Process.Start("CMD.exe", "/c adb pull " + textBox6.Text);
            process.WaitForExit();
        ...
        {
            var process = Process.Start("CMD.exe", "/c adb push " + textBox4.Text + " " + textBox5.Text);
            process.WaitForExit();
        ...
        private void button12_Click(object sender, EventArgs e)
        {
            ...
            {
                var process = Process.Start("CMD.exe", "/c adb shell " + textBox7.Text);
                process.WaitForExit();
            }
            if (checkBox3.Checked == false)
            {
                var process = Process.Start("CMD.exe", "/c adb " + textBox7.Text);
                process.WaitForExit();
            }
        }
        ...
        {
            var process = Process.Start("CMD.exe", "/c adb sideload " + textBox8.Text);
            process.WaitForExit();
